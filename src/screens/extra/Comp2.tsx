...
// import { useSearchAllProductsMutation, useSearchFiltersMutation } from "../../../../Redux/Api/productAPI";
...
const Comp2 = () => {
  ...
  const [combinedState, setCombinedState] = useState<any>({
    search: "",
    rating: null,
    company: null,
    category: "TMT BAR K",
  ...
  //  console.log(SearchProductsResponse, "hundred");

  useEffect(() => {
    const searchTerm = "UP2 (GREY) AC RIDGE EVEREST";
    searchAllProducts( searchTerm );
  }, []);
  
  // console.log("second result",Results)
    const id = SearchProductsResponse?.products[0]?._id 
    ...
    const [priceMutation, { data: PriceResponse }] = usePriceMutation();
    ...
    }, [id]); // Dependency array ensures this useEffect runs only when `id` changes
  ...
  {SearchProductsResponse?.products[0]?.productName}
</div>
      ...
      <div className=" w-full relative ">
        ...
        <div className="absolute top-[0.34rem] left-[0.21rem] sm:left-[2rem] md:top-[4rem]  md:left-[2rem] lg:top-[5rem]  lg:left-[16rem] w-[15rem] md:w-[25rem]  lg:w-[25rem] h-full sm:h-[21rem] lg:h-[70rem]">
          <div className="relative  lg:mt-10 z-0 ">
            <img
              src={SearchProductsResponse?.products[0]?.images}
              className="absolute -top-20 right-16 z-10 w-[7.78rem] border rounded-xl shadow-sm md:w-[27rem]" 
